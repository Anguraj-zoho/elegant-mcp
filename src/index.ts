...
async function handleGetComponent(args: { name: string }): Promise<string> {
  ...
  if (!entry) return `Unknown component: ${args.name}. Use list_components to see options.`;

  const content = await readWikiFile(entry.file);
  if (!entry.section) return content;

  const html = rewriteAssetPaths(extractHtmlBlock(content, entry.section));
  return `## ${args.name} — ${entry.section}\n\nSource: ${entry.file}\nCDN: ${CDN_BASE}\n\n\`\`\`html\n${html}\n\`\`\``;
}

function handleGetRecipe(args: { shell: string }): string {
  const r = RECIPES[args.shell.toUpperCase()];
  if (!r) return `Unknown shell: ${args.shell}`;
  return r;
}

function handleGetShell(args: { shell: string }): string {
  const s = SHELLS[args.shell.toUpperCase()];
  if (!s) return `Unknown shell: ${args.shell}`;
...
function handleGetChecklist(args: { shell: string }): string {
  ...
  if (!c) return `No checklist for shell: ${args.shell}`;
  return c;
}

function handleListComponents(): string {
...
async function handleSearchWiki(args: { query: string; max_results?: number }): Promise<string> {
...
function handleGetIcons(args: { filter?: string }): string {
  let icons = ALL_ICONS;
  ...
  if (icons.length === 0) return `No icons found matching: "${args.filter}"`;

  const withCdn = icons.map(i => `${i}  →  ${CDN_BASE}/assets/icons/${i}`);
  return `${icons.length} icon(s):\n\n${withCdn.join("\n")}`;
}

async function handleGetFullWikiFile(args: { filename: string }): Promise<string> {
  return rewriteAssetPaths(await readWikiFile(args.filename));
}

function handleGetAntiPatterns(): string {
  return ANTI_PATTERNS;
}

function handleSetupProject(args: { feature_name: string }): string {
...
async function handleGetTopnavbar(): Promise<string> {
  const entry = COMPONENT_MAP["topnavbar"];
  if (!entry) return "TopNavBar not found in component map";
  const content = await readWikiFile(entry.file);
...
function handleGetPageBlueprint(args: { page: string }): string {
  ...
  if (!bp) return `Unknown page: ${args.page}. Options: ${Object.keys(PAGE_BLUEPRINTS).join(", ")}`;
  return bp;
}
...
const server = new Server(
  { name: "elegant-agent-mcp", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    switch (name) {
      ...
      case "search_wiki":        text = await handleSearchWiki(args as { query: string; max_results?: number }); break;
      case "get_icons":          text = handleGetIcons(args as { filter?: string }); break;
      case "get_full_wiki_file": text = await handleGetFullWikiFile(args as { filename: string }); break;
